...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    image_storage: {
      ...
      webdav_root_path: String(imageStorage.webdav_root_path || "webchat2api/images"),
    ...
    backup: {
      ...
      encrypt: Boolean(backup.encrypt ?? true),
      passphrase: String(backup.passphrase || ""),
      include: {
        config: Boolean(backup.include?.config ?? false),
        cpa: Boolean(backup.include?.cpa ?? false),
        sub2api: Boolean(backup.include?.sub2api ?? false),
        logs: Boolean(backup.include?.logs ?? true),
        image_tasks: Boolean(backup.include?.image_tasks ?? true),
        accounts_snapshot: Boolean(backup.include?.accounts_snapshot ?? false),
        auth_keys_snapshot: Boolean(backup.include?.auth_keys_snapshot ?? false),
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        image_storage: {
          ...
          webdav_root_path: String(config.image_storage?.webdav_root_path || "webchat2api/images").trim(),
